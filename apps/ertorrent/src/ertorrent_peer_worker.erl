...
-record(incoming, {piece_blocks::list(),
                   piece_hash,
                   piece_index,
...
-record(outgoing, {outgoing_piece_data::binary(),
                   outgoing_piece_hash}).
...
complete_piece(State) ->
    Blocks = State#state.state_incoming#incoming.piece_blocks,
...
terminate(Reason, State) ->
    ...
    erlang:cancel_timer(State#state.keep_alive_external_ref),
    erlang:cancel_timer(State#state.keep_alive_internal_ref),
...
handle_info({peer_srv_piece, Index, Hash, Data}, State) ->
...
handle_info({tcp, S, <<?BITFIELD, Bitfield/binary>>}, State) ->
    ?INFO("peer[" ++ State#state.address ++ "] received bitfield " ++ Bitfield),
    {noreply, State};
